...
async def safe_send(interaction: discord.Interaction, message: str):
    try:
...
@app_commands.describe(
    ...
    uidtag="UID bị tag (cách nhau bằng dấu phẩy)",
...
async def nhaytagmess(
    ...
    try:
        ...
        return await interaction.followup.send("❌ Không tìm thấy file `nhay1.txt`!")

    if not lines:
        return await interaction.followup.send("❌ File `nhay1.txt` không có nội dung!")

    try:
        user_id, fb_dtsg, rev, req, a, jazoest = get_auth_tokens(cookie)
    except Exception as e:
        return await interaction.followup.send(f"❌ Không thể lấy thông tin Facebook: {e}")

    if not all([user_id, fb_dtsg]):
        return await interaction.followup.send("❌ Cookie không hợp lệ hoặc đã hết hạn.")

    # --- Xử lý nhiều UID ---
    import re
    uidtags = re.split(r"[,\s]+", uidtag.strip())
    uidtags = [u for u in uidtags if u.isdigit()]
    if not uidtags:
        return await interaction.followup.send("❌ Bạn chưa nhập UID hợp lệ để tag!")

    # Lấy thông tin tên cho từng UID
    tag_info_list = []
    for uid in uidtags:
        user_data = fetch_user_info(uid, cookie)
        if "error" in user_data:
            tag_info_list.append((uid, f"UID:{uid}"))  # fallback
        else:
            tag_info_list.append((uid, user_data.get("name", f"UID:{uid}")))
    ...
    def nhaytag_worker():
        ...
        while not stop_event.is_set():
            for line in lines:
                ...
                for uid, tag_name in tag_info_list:
                    if stop_event.is_set():
                        break
    ...
    await interaction.followup.send(
        f"✅ Đã bắt đầu spam tag {len(uidtags)} người trong box `{idbox}` mỗi `{delay}`s\n"
        f"• Bắt đầu: `{start_time.strftime('%Y-%m-%d %H:%M:%S')}`"
    )
...
async def tabnhaytagmess(interaction: discord.Interaction):
    ...
    if not tabs:
        return await safe_send(interaction, "❌ Không có tab spam tag nào đang chạy.")
    ...
    try:
        ...
        return await interaction.followup.send("⏱️ Hết thời gian.")
    ...
    await interaction.followup.send(f"⛔ Đã dừng tab spam tag số `{i}`")                                                                                                                                    
@tree.command(
    name="tabnhaytop",
...
@tree.command(name="treomess", description="Treo tin nhắn Messenger bằng nội dung từ file.txt đã upload")
@app_commands.describe(
    idbox="ID Box",
    cookie="Cookie Facebook",
    noidung="Tên file .txt đã lưu bằng /setngonmess (không cần đuôi .txt)",
...
async def treomess(interaction: discord.Interaction, idbox: str, cookie: str, noidung: str, delay: float):
    ...
    try:
        ...
        return await interaction.response.send_message(f"❌ Cookie không hợp lệ hoặc lỗi: {e}")

    # Kiểm tra và tải nội dung từ file .txt
    filename = noidung if noidung.endswith(".txt") else f"{noidung}.txt"
    file_path = os.path.join(NGONMESS_DIR, f"{discord_user_id}_{filename}")

    if not os.path.exists(file_path):
        return await interaction.response.send_message(f"❌ File `{filename}` không tồn tại.")

    with open(file_path, "r", encoding="utf-8") as f:
        content = f.read().strip()

    if not content:
        return await interaction.response.send_message("❌ File trống!")

    def get_message():
        return content  # Gửi toàn bộ nội dung file mỗi lần
...
async def add(interaction: discord.Interaction, user: str, thoihan: str = None):
    ...
    if thoihan and thoihan.endswith("d"):
        try:
            ...
            return await interaction.response.send_message("Thời hạn không hợp lệ. Phải là số + 'd'.")
...
async def list_cmd(interaction: discord.Interaction):
    ...
    # Ảnh góc phải
    embed.set_thumbnail(url="https://media.discordapp.net/attachments/1389594739753484511/1411763880039944282/13474aab4bc986442fcbb22b1d7fda4e.jpg?ex=68b5d712&is=68b48592&hm=58935f6c6a4a7ba09127b6bbf1f08ead23238e3ac55fcf1a5c14ef9e03c0a9a5&=&format=webp&width=569&height=569")
...
class TreoDisModal(discord.ui.Modal):
    token = discord.ui.TextInput(
        label="⚡Token (Nhập token discord của bạn)", 
        style=discord.TextStyle.paragraph,
        required=True  # Chắc chắn người dùng phải nhập token
    )
    channel_id = discord.ui.TextInput(
        label="⚡Channel ID (Nhập ID kênh cần spam)",
        required=True  # Chắc chắn người dùng phải nhập Channel ID
    )
    message = discord.ui.TextInput(
        label="⚡Nội dung (Tin nhắn cần spam)",
        style=discord.TextStyle.paragraph,
        required=True  # Chắc chắn người dùng phải nhập nội dung tin nhắn
    )
    delay = discord.ui.TextInput(
        label="⚡Delay (Giây)",
        required=True  # Chắc chắn người dùng phải nhập delay
    )

    def __init__(self):
        super().__init__(title="🌀Tool Treo Discord🌀")

    async def on_submit(self, interaction: discord.Interaction):
        # Lấy dữ liệu từ các trường nhập của modal
        token = self.token.value.strip()
        channel_id = self.channel_id.value.strip()
        message = self.message.value.strip()
        delay = float(self.delay.value.strip())  # Chuyển đổi delay thành kiểu số

        # Kiểm tra quyền người dùng
        if not is_authorized(interaction) and not is_admin(interaction):
            return await interaction.response.send_message("Bạn không có quyền sử dụng bot")

        # Tạo danh sách các token, channel_id và delay
        tokens_list = [token]
        ...
        # Thông báo người dùng về kết quả
        return await interaction.response.send_message(
...
@tree.command(name="treodis", description="Spam 1 nội dung lên Discord")
async def treodis(interaction: discord.Interaction):
    # Khi người dùng nhập lệnh /treodis, modal sẽ hiện lên
    await interaction.response.send_modal(TreoDisModal())
  
@tree.command(
    name="tabtreodis",
    description="Quản lý/dừng tab treo discord"
)
async def tabtreodis(interaction: discord.Interaction):
    ...
    discord_user_id = str(interaction.user.id)
    async with DIS_LOCK:
        tabs = list(user_discord_tabs.get(discord_user_id, []))  # copy an toàn
    ...
    for idx, tab in enumerate(tabs, 1):
        # an toàn lấy dữ liệu (tránh KeyError / kiểu không mong muốn)
        channels = ",".join(map(str, tab.get("channels", []))) or "—"
        delays = ",".join(map(str, tab.get("delays", []))) or "—"
        tokens_obj = tab.get("tokens", [])
        tokens_count = len(tokens_obj) if hasattr(tokens_obj, "__len__") else 0

        # hỗ trợ cả trường hợp tab["start"] là datetime hoặc float timestamp
        ...
        msg += (
            f"{idx}. Channels:`{channels}` | "
            f"Tokens:`{tokens_count}` | Delays:`{delays}`s | "
            f"Uptime:`{uptime}`\n"
        )
    ...
    # Dừng tab an toàn
    async with DIS_LOCK:
        # lấy lại danh sách hiện tại (tránh race)
        current_tabs = user_discord_tabs.get(discord_user_id, [])
        if not current_tabs or i > len(current_tabs):
            return await interaction.followup.send("Tab đã thay đổi, thử lại.", ephemeral=True)

        tab = current_tabs.pop(i - 1)
        # hủy task nếu có
...
# Slash command chính
@tree.command(name="nhaydis", description="Tạo tab spam chửi trên Discord")
async def nhaydis(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "- mỗi đối tượng sẽ có 1 ngôn chửi riêng✅\n"
            "- dùng lệnh /tabnhaydis để dừng bot spam⛔\n"
            f"Admin điều hành bot: <@{admin_id}>"
        ),
...
async def tabnhaydis(interaction: discord.Interaction):
    ...
    if not tabs:
        return await interaction.response.send_message("❌ Bạn không có tab spam Discord nào đang hoạt động.")

    msg = "**Danh sách tab spam Discord của bạn:**\n"
    for idx, tab in enumerate(tabs, 1):
        uptime = get_uptime(tab["start"])
    ...
    await interaction.followup.send(f"⛔ Đã dừng tab spam số `{i}`")
...
async def menu(interaction: discord.Interaction):
    ...
    # Ảnh góc phải (thumbnail)
    embed.set_thumbnail(url="https://media.discordapp.net/attachments/1402549122803961856/1411390579027349504/1bac0a4a7bad8bf88fb6efbb7c664db6.jpg?ex=68b47b68&is=68b329e8&hm=f3011c416fe258d3ba65b81655416ffc81c1e89f4a8ff91558e42cef3c5556ed&=&format=webp&width=569&height=569")


    # Ảnh động dưới cùng
    embed.set_image(url="https://media.discordapp.net/attachments/1369607109779394693/1385867763985154108/305875b6dbbdcefb27661f50a6d6a9be.gif?ex=68b53bef&is=68b3ea6f&hm=98b2fcb399308e719cfa6e155e1fd1c283b2a61079f62aa692361fc9ebc219e6&=")

    embed.set_footer(text="👉 Chúc bạn sàn war vui vẻ 🔥.\n🛠 Bot war")

    # Tạo view với 3 nút màu
    class MenuView(View):
        def __init__(self):
            super().__init__(timeout=None)

        @discord.ui.button(label="dz thì ấn vô đây", style=discord.ButtonStyle.danger)
        async def msg_btn1(self, interaction: discord.Interaction, button: Button):
            await interaction.response.send_message (" dz vl <@1484506341404835912>]")
        ...
        async def msg_btn2(self, interaction: discord.Interaction, button: Button):
            await interaction.response.send_message("BOT WAR DISCORD VÀ LÀ CHA CỦA LŨ ĐÚ 2026")
        ...
        async def help_btn(self, interaction: discord.Interaction, button: Button):
            help_embed = discord.Embed(
                ...
                description=(
                    " > **🌟Lệnh cơ bản**\n\n"
                    " Gõ lệnh `/menu` để mở menu\n"
                    " Gõ lệnh `/add` để thêm người có quyền sử dụng bot <chỉ dùng cho admin>\n"
                    " Gõ lệnh `/xoa` để xóa quyền người sử dụng bot <chỉ dùng cho admin>\n\n"
                    " > **✅Lệnh khởi động chức năng**\n\n"
                    " Gõ lệnh `/treoanhmess` để spam ảnh trên mess\n"
                    " Gõ lệnh `/reostr` để réo người khác trên str fb\n"
                    " Gõ lệnh `/nhaytop` để chửi trên bài viết fb\n"
                    " Gõ lệnh `/treomess` để spam 1 nội dung trên mess\n"
                    " Gõ lệnh `/nhaymess` để spam chửi trên box mess\n"
                    " Gõ lệnh `/nhaytagmess` để soam chửi và tag người cần chửi mess\n"
                    " Gõ lệnh `/nhaynamebox` để spam chửi bằng tên nhóm mess\n"
                    " Gõ lệnh `/treodis` để spam 1 nội dung trên discord\n"
                    " Gõ lệnh `/nhaydis` để chửi trên discord\n"
                    " Gõ lệnh `/treozalo` để treo 1 nội dung trên zalo\n"
                    " Gõ lệnh `/nhaytagzalo` để dừng chửi tag trên zalo\n"
                    " Gõ lệnh `/treopollzl` để spam bình trọn zalo\n"
                    " Gõ lệnh `/treotele` để treo nội dung trong telegram\n"
                    " Gõ lệnh `/nhaynameboxzl` để chửi trên tên box zalo\n"
                    " Gõ lệnh `/treosticker` để spam sticker zalo\n"
                    " Gõ lệnh `/treoig` để treo bot INSTAGRAM\n"
                    " Gõ lệnh `/treogmail` để spam gmail\n"
                    " Gõ lệnh `/treosms` để spam sms\n"
                    " Gõ lệnh `/treowechat` để spam tin nhắn we chat\n"
                    " Gõ lệnh `/checkcookie` để check cookie mess\n\n"
                    " > **⛔Lệnh dừng chức năng**\n\n"
                    " Gõ lệnh `/tabtreodis` để dừng tab treo discord\n"
                    " Gõ lệnh `/tabnhaydis` để dừng tab chửi trên discord\n"
                    " Gõ lệnh `/tabnhaymess` để dừng tab chửi mess\n"
                    " Gõ lệnh `/tabnhaytagmess` để dừng tab chửi tag trên mess\n"
                    " Gõ lệnh `/tabnhaynamebox` để dừng tab chửi trên tên box\n"
                    " Gõ lệnh `/tabreostr` để dừng tab réo str\n"
                    " Gõ lệnh `/tabtreozalo` để dừng treo trên zalo\n"
                    " Gõ lệnh `/tabnhaynameboxzl` để dừng chửi tên box zalo\n"
                    " Gõ lệnh `/tabtreosticker` để dừng spam sticker zalo\n"
                    " Gõ lệnh `/tabtreoig` để dừng treo bot INSTAGRAM\n"
                    " Gõ lệnh `/tabtreogmail` để dừng spam gmail\n"
                    " Gõ lệnh `/tabtreosms` để dừng spam sms\n"
                    " Gõ lệnh `/tabwechat` để dừng spam wechat\n\n"
                    "💡 **All lệnh bot war**"
                ),
                color=0x1E90FF
            )
        ...
        async def info_btn( interaction: discord.Interaction, button: Button):
            info_embed = discord.Embed(
                ...
                description=(
                    ...
                    "📅 Ngày ra mắt: 1/1/2023\n"
                    
                ),
                color=0x32CD32
            )
            info_embed.set_footer(text="Bot War - Powered by Discord")
